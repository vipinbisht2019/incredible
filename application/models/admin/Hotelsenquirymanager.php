<?php 
   ...
   */
   class Hotelsenquirymanager extends CI_Model
   {
	  ...
	  //-----------------------------------------add------------------------------------------------			 
       ...
       //------------------------------------view-----------------------------------------------------		
		    ...
		    function hotels_view($limit,$offset){
		      ...
		      public function hotels_details($limit,$offset)
	          {

					  $this->db->select('*');
					  $this->db->from('usc_enquiry as enquiry')
							->join("usc_hoteles as hoteles", "enquiry.Id = hoteles.HotelId", 'INNER')     
							->order_by("enquiry.Enq_id", "desc")
							->where('enquiry.Enq_type', '6');
