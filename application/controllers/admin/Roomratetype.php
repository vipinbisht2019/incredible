...
class  Roomratetype extends MY_controller {
...
// ------------------------------- View funnctin load listing page while clicking on management ------------------------------
	public function view()
	  {
		        $this->load->model('admin/Roomratetypemanage');
		   
		        $this->load->library('pagination');
				$config['base_url'] = base_url('admin/roomratetype/view');
				$config['per_page'] = 50; 
				$config['total_rows'] =$this->Roomratetypemanage->get_tatal();
		   ...
		   $data['roomtype']=$this->Roomratetypemanage->roomratetype_view($config['per_page'] , $offset);
		   $this->load->view('admin/roomratetype_view',$data);
	   ...
	   if($this->input->post('flag')=='yes') 
		    {
		       ...
		       // -------------------------- form vaildation ---------------------------------------
						...
						$this->form_validation->set_rules('room_rate_type', 'room rate type', 'required');
				...
				if($this->form_validation->run() == true) 
		             {
						   $this->load->model('admin/Roomratetypemanage');
							...
							$insert=$this->Roomratetypemanage->roomratetype_add($data);
							if($insert==1)
							 {
								$this->session->set_flashdata('roomratetype', 'Records added successfully');
								redirect(base_url('admin/roomratetype/view'));
							 }
					   ...
					   { 
					       ...
					       $this->load->model('admin/Roomratetypemanage');
						   $data['hotels']=$this->Roomratetypemanage->roomtype_hotel(); 
						 
					        $this->load->view('admin/roomratetype_add',$data);
					   }  			
					...
					if($this->form_validation->run() == true) 
					  {
								...
								//------------------------------------ END Upload ---------------------------------------------------
									 ...
									 unset($data['smt_enter']);
									 unset($data['id']);
									 $this->Roomratetypemanage->roomtype_edit_data($data,$id);
									 $this->session->set_flashdata('roomratetype', 'Records updated successfully.');
									 redirect(base_url('admin/roomratetype/view'));
					  }
					 else
					  {
						   $data['edit_roomtype']=$this->Roomratetypemanage->roomtype_edit($id);
						   //$data['hotels']=$this->Roomratetypemanage->roomtype_hotel(); 
						   $this->load->view('admin/roomratetype_modify',$data);
					  } 	  
	...
	public function delete()
	  {
			$this->load->model('admin/Roomratetypemanage');
			 $id=$this->input->get('id');
			$delete_sussess=$this->Roomratetypemanage->roomtype_delete($id)  ; 
		
		    if($delete_sussess==1)	
		     {			
			   $this->session->set_flashdata('roomratetype', 'Records deleted successfully');
		       redirect(base_url('admin/roomratetype/view'));
		     }	
