<?php 
   ...
   */
   class Carenquirymanager extends CI_Model
   {
	  ...
	  //-----------------------------------------add------------------------------------------------			 
       ...
       //------------------------------------view-----------------------------------------------------		
		    ...
		    function enquiry_view($limit,$offset){
		      ...
		      public function car_details($limit,$offset)
	          {

					  $this->db->select('*');
					  $this->db->from('usc_enquiry as enquiry')
							->join("usc_cardetails as cardetails", "enquiry.Id = cardetails.CarId", 'INNER')     
							->order_by("enquiry.Enq_id", "desc")
							->where('enquiry.Enq_type', '2');
