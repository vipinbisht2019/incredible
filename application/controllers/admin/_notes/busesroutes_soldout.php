...
// ------------------------------- View funnctin load listing page while clicking on management ------------------------------
	public function view(){
			...
			//---------------------------- optional parameters of paging  ---------------------------------------------------	
		     ...
		     $data['routedetails_boarding']=$this->Busesroutes_soldoutmanage->buses_routesdetails_boarding($config['per_page'] , $offset,$routeid,$BusesRoutsId);
			 //$data['routedetails']=$this->busesroutes_soldoutmanage->buses_routesdetails($routeid);
		  ...
		  if($this->input->post('flag')=='yes') 
		          {
									       ...
									       if($SBoardingIdArr[$city_id][$b_id]!='')
											       {
														...
														$this->Busesroutes_soldoutmanage->buses_routesdetails_boarding_add($data);
				   ...
				   else
					 {
					      ...
					      $data['route_cities']=$this->Busesroutes_soldoutmanage->buses_routesdeatils_broading($bid,$rid);
				   ...
				   if($this->input->post('flag')=='yes') 
					 {
								   ...
								   $RoutesId=$this->input->post('RoutesId');
								   $BusesRoutsId=$this->input->post('BusesRoutsId');
								   $this->Busesroutes_soldoutmanage->buses_routesdetails_boarding_delete($BusesRoutsId);
								   for($ii=0;$ii<count($CityIdArr);$ii++)
									   {
											 $city_id=$CityIdArr[$ii];
											 $BoardingArr=$BoardingPointsArr[$city_id];
				  ...
				  else
					{
								 ...
								 $data['route_cities']=$this->Busesroutes_soldoutmanage->buses_routesdeatils_broading($bid,$rid);
								  ...
								  $data['route_cities_boarding']=$this->Busesroutes_soldoutmanage->buses_routesdeatils_broading_edit($bid,$rid);
							      $this->load->view('admin/buses_routedetails_soldout_modify',$data);
