<?php 
   ...
   */
   class Tourenquirymanager extends CI_Model
   {
	  ...
	  //-----------------------------------------add------------------------------------------------			 
       ...
       //------------------------------------view-----------------------------------------------------		
		    ...
		    function tour_view($limit,$offset){
		      ...
		      public function tour_details($limit,$offset)
	          {

					  $this->db->select('*');
					  $this->db->from('usc_enquiry as enquiry')
							->join("usc_tourgeneralinfo as info", "enquiry.Id = info.TourId", 'INNER')     
							->order_by("enquiry.Enq_id", "desc")
							->where('enquiry.Enq_type', '1');
