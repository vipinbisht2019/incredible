   ...
   //--------------------------------------------------------------- Cansllation Stuff Start -------------------------------------------------	
	...
	//------------------------------------------------------ Update booking -----------------------------------------
		    ...
		    function tours_list_dropdown()
				...
				public function tourpackage_termcondition_print($id)
			    {
						  
					 $this->db->select('terms_c.Title')
						  ->from('usc_termscondition as terms_c')
						  ->join("usc_tourtermconditions as terms_t", "terms_c.TermsconditionID = terms_t.TermsconditionID", 'INNER')
						  ->where('TourId', $id);
					$query = $this->db->get();
					return $query->result_array();
					$this->db->close();
