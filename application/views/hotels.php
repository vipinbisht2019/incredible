...
$this->session->set_userdata('guestsinfo', $guestInfo);
$this->session->set_userdata('noofrooms', $room);
$this->session->set_userdata('noofadults', $adults);
$this->session->set_userdata('noofchild', $child);
...
section.popular-packages{ margin-top: 0px; }
    </style>
  </head>
    ...
    <div id="main">
        ...
        <div class="container">
          <form class="form-inline flight-flex" name="searchhotel" id="searchhotel" method="post" action="<?php echo base_url('hotels'); ?>">
            ...
            <div class="form-group" style="padding: 5px;">
              ...
              <input type="text" name="checkindate" id="start-date" placeholder="Start date" value="<?php echo $checkindate; ?>" style="border-radius: 6px;background: #3658a9;border: #3658a9;color: #fff;"/>
            </div>
            <div class="form-group" style="padding: 5px;">
              <label for="date" class="flight-label">Checkout</label><br>
              <input type="text" name="checkoutdate" id="end-date" placeholder="End date"  value="<?php echo $checkoutdate; ?>"  style="border-radius: 6px;background: #3658a9;border: #3658a9;color: #fff;"/>
      ...
      <div class="container">
        <div class="row">
         ...
         <?php include('includes/hotel_left_menu.php'); ?>
          ...
          <div class="col-sm-9">
            <div style="position:sticky;top:123px;z-index: 2;background: #fff;">
                ...
                <ul style="list-style:none;display: flex;padding-top:2px;padding-bottom: 2px;border-top: 1px solid #ddd;border-bottom: 1px solid #ddd;">
                  ...
                  <li style="flex:1;font-size:12px;justify-content: end;display: flex;align-items: center;right: 70px;position: relative;">Price &nbsp;<i class="fa fa-caret-down" id="rotate2" aria-hidden="true"></i></li>
                </ul>
            </div>
            <div class="row">
            <?php 
                                    ...
                                    <div class="tooltip-container">
                                      <div class="fw-price">
                                          ...
                                          <p> 
                                            ...
                                            <span style="font-size: 20px;"><i class="fa fa-inr"></i><?php echo round($hotelValue['totalPrice']); ?></span>
                                          </p>
                                      ...
                                      <?php //echo base_url('hotels/booking_hotels?hotelId='.$hotelValue['hotelId'].'room='.$room.'adult='.$adults.'child='.$child); ?>
                                    <a href="<?php echo base_url('hotels/booking_hotels?hotelId='.$hotelValue['hotelId'].'&adult='.$adults.'&child='.$child.'&room='.$room.'&guestInfo='.$guestInfo) ?>" class="btn btn-red btn-clr" tabindex="0" target="_blank">Select Room</a>
                                  </div>
                                </div>
                              </div></div></div>
                              </div>
  ...
  $(function() {
      $( "#slider-range" ).slider({
          ...
          slide: function( event, ui ) {
              ...
              $("#hotel_minimum_price").val(ui.values[ 0 ]);
              $("#hotel_maximum_price").val(ui.values[ 1 ]);
    ...
    $("#amount").val( "" + $( "#slider-ranges" ).slider( "values", 0 ) + " - " + $( "#slider-ranges" ).slider( "values", 1 ) );
  });
</script>
<script>
...
(function() {
  ...
  <script>
    function getPage(){
      ...
      var max11=$('#hotel_minimum_price').val();
      var min11=$('#hotel_maximum_price').val();

      
      if(max11 != '') {   
      var minimum_price = $('#hotel_minimum_price').val();
      }
      else {
        var minimum_price = '';
      }
      ...
      if(min11 != '') { 
      var maximum_price = $('#hotel_maximum_price').val();
      } else {
          var maximum_price = '';
      }
      ...
      $.each($("input[name='rating']:checked"), function(){            
        rating_id.push($(this).val());
      });
      var myrating_id=rating_id.join(", ");
      if(myrating_id!='') { var ratingId=myrating_id; } else { var ratingId="0"; } 

      var meals_id = [];
      $.each($("input[name='meals']:checked"), function(){            
        meals_id.push($(this).val());
      });
      var mymeals_id=meals_id.join(", ");
      if(mymeals_id!='') { var mealsId = mymeals_id; } else { var mealsId="0"; } 

      var property_id = [];
      $.each($("input[name='property']:checked"), function(){            
        property_id.push($(this).val());
      });
      var myproperty_id=property_id.join(", ");
      if(myproperty_id!='') { var propertyId = myproperty_id; } else { var propertyId="0"; } 

      var cityName = '<?php echo $cityName; ?>'; 
      var checkinDate = '<?php echo $checkindate; ?>';
      var checkoutDate = '<?php echo $checkoutdate; ?>';
      var adults = '<?php echo $adults; ?>';
      var childs = '<?php echo $child; ?>';
      var childAge = '<?php echo $childAge; ?>';
      var specialCategory = '<?php echo $specialCat; ?>';

      var hotelnm = document.getElementById('hotel_name').value;
      if(hotelnm!='') { var hotelName = hotelnm; } else { var hotelName="0"; } 
      
      // var ratings = '<?php //echo $rating; ?>'; 
      // alert(ratings); 

      $('#hotelData').html('<center> <img src="<?php echo base_url('assets/images/loadanimation.gif') ?>"></center>');  
      jQuery.ajax({
        url: "<?php echo base_url('hotels/hotel_filter')?>",
              type: 'post',
              dataType: "html",
              data: {'ratingId':ratingId,'mealsId':mealsId,'propertyId':propertyId,'cityname':cityName,'checkinDate':checkinDate,'checkoutDate':checkoutDate,'adults':adults,'childs':childs,'specialCategory':specialCategory,'childAge':childAge,'hotelName':hotelName,'minimum_price':minimum_price, 'maximum_price':maximum_price},
              success: function( data ) {
                //console.log(data);
                $("#hotelData").html(data);
              }
    ...
    function getHotelList(){
      ...
      jQuery.ajax({
        url: "<?php echo base_url('hotels/hotel_filter')?>",
              type: 'post',
              dataType: "html",
              data: {'ratingId':ratingId,'mealsId':mealsId,'propertyId':propertyId,'cityname':cityName,'checkinDate':checkinDate,'checkoutDate':checkoutDate,'adults':adults,'childs':childs,'specialCategory':specialCategory,'childAge':childAge,'hotelName':hotelName},
              success: function( data ) {
                //console.log(data);
                $("#hotelData").html(data);
              }
    ...
    <script type='text/javascript'>
        ...
        $(document).ready(function() { 
          ...
          $( "#search_hotelname" ).autocomplete({
            ...
            select: function (event, ui) {
              $('#search_hotelname').val(ui.item.label); // display the selected text
              $('#search_hotelname').val(ui.item.value); 
              return false;
            }
