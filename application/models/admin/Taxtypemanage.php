...
*
*/
   class Taxtypemanage extends CI_Model
   {
      
	  //-----------------------------------------add------------------------------------------------			 
			function taxtype_add($data)
			 {
			  // Inserting in Table(usc_hotelsroomtype) of Database(usc)
			  
				 if($this->db->insert('usc_taxtype', $data))
					 {
					   return 1;
					 }
					else
       ...
       //------------------------------------view-----------------------------------------------------		
		    ...
		    function taxtype_view($limit,$offset)
			 {
			   // Updateing in Table(usc_hotelsroomtype) of Database(usc)
			   
				
			       $this->db->from('usc_taxtype',$limit,$offset)
				           
			    	        ->order_by("usc_taxtype.SortOrder");
