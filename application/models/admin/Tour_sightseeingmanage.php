	     ...
	     //------------------------------------edit date-----------------------------------------------------		
		   ...
		   public function tour_hotels_add($data)
			...
			public function getIteneraryCity($tourId){
				...
				// $this->db->join('usc_toursightseeings_info as tourSight','tourSight.ItenaryDay = Iti.ItenaryDay');
				// $this->db->join('usc_toursightseeings_moreinfo as tourMoreSight','tourMoreSight.s_info_id = tourSight.id');
				$this->db->join('usc_city as city','Iti.cityid = city.cityid');
				$this->db->where('Iti.TourId',$tourId);
				//$this->db->where('tourSight.TourId',$tourId);
				$this->db->order_by('Iti.ItenaryDay');
				$query = $this->db->get()->result_array();

				foreach ($query as $key => $sightValue) {

					$this->db->select('tourSight.id,tourSight.ItenaryDay,tourSight.sightSeeing,tourSight.Time');
					$this->db->from('usc_toursightseeings_info as tourSight');
					$this->db->where('TourId',$tourId);
					$this->db->where('ItenaryDay',$sightValue['ItenaryDay']);
					$query[$key]['tourSightSeeingList'] = $this->db->get()->result_array();

					foreach ($query[$key]['tourSightSeeingList'] as $key1=> $sightsValue) {

						$this->db->select('tourMoreSight.sightSeeings as sightSeeing,tourMoreSight.times as Time');
						$this->db->from('usc_toursightseeings_moreinfo as tourMoreSight');
						$this->db->where('s_info_id',$sightsValue['id']);
						//$this->db->where('itineraryDay',$sightsValue['ItenaryDay']);
						$query[$key]['tourSightSeeingList'][$key1]['tourSightSeeingMoreList'] = $this->db->get()->result_array();
			...
			public function submitSightSeeing($data){
			...
			public function getSightSeeingDetail($tourId){
			...
			public function tour_sight_delete($id)
			 {
